...
async def test_health_check(async_client):
    ...
    with patch(
        "aistudioproxy.api.routes.browser_manager", new_callable=AsyncMock
...
async def test_chat_completions_streaming(async_client):
    ...
    with patch(
        "aistudioproxy.api.routes.request_handler.handle_stream_request",
        new=mock_stream_response,
    ):
        response = await async_client.post(
            "/v1/chat/completions",
            json={
...
async def test_health_check_browser_error(async_client):
    ...
    """
    with patch(
        "aistudioproxy.api.routes.browser_manager.health_check",
