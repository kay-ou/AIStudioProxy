...
@pytest.fixture
def app_with_mocks(
    test_config, mock_browser_manager, mock_request_handler, mock_auth_manager
):
    ...
    with (
        patch("aistudioproxy.api.app.get_config", return_value=test_config),
...
@pytest.fixture
def app_for_integration(test_config, mock_browser_manager, mock_auth_manager):
    """
    Create a FastAPI app for integration testing.

    This fixture uses a real RequestHandler but mocks the underlying browser
    and auth managers. This allows for testing the full API and handler logic
    ...
    with (
        ...
        set_dependencies(
            handler=handler,  # Use the real handler
            browser=mock_browser_manager,
            auth=mock_auth_manager,
        )
...
@pytest.fixture
async def integration_client(app_for_integration) -> AsyncGenerator[AsyncClient, None]:
    """Create an async test client for integration tests."""
    async with AsyncClient(app=app_for_integration, base_url="http://test") as ac:
        yield ac
