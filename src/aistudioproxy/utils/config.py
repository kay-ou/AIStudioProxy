...
class BrowserConfig(BaseSettings):
    ...
    port: int = Field(
        default=9222, ge=1024, le=65535, description="Browser debugging port"
    )
    timeout: int = Field(
        default=30000, ge=1000, description="Browser operation timeout in ms"
    )
    ...
    initial_pool_size: int = Field(
        default=5, ge=1, le=50, description="Initial number of pages in the page pool"
    )
...
class AuthConfig(BaseSettings):
    ...
    profile_path: Optional[str] = Field(
        default=None, description="Browser profile path"
    )
    ...
    file_path: Optional[str] = Field(
        default=None, description="Path to the authentication file (e.g., cookies.json)"
    )
...
class PerformanceConfig(BaseSettings):
    ...
    max_concurrent_requests: int = Field(
        default=50, ge=1, le=1000, description="Maximum concurrent requests"
    )
    request_timeout: int = Field(
        default=60, ge=5, description="Request timeout in seconds"
    )
    ...
    cleanup_delay: int = Field(
        default=300,
        ge=0,
        description="Delay in seconds before cleaning up request tracking",
    )
