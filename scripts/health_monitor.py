...
class HealthMonitor:
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "unhealthy", "error": f"Connection error: {e}"}
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(f"{self.api_url}/metrics") as response:
                    if response.status == 200:
                        ...
                        return {"status": "available", "data": metrics_data}
                    else:
                        return {
                            "status": "unavailable",
                            "error": f"HTTP {response.status}",
    ...
    async def perform_recovery_action(self):
        ...
        logger.error(
            f"Last health check result: {json.dumps(health_result, indent=2)}"
        )
    ...
    async def log_health_status(
        self, health_result: Dict[str, Any], metrics_result: Dict[str, Any]
    ):
        ...
        if status == "healthy":
            ...
            error_msg = health_result.get('error', 'unknown')
            logger.warning(
                f"Service unhealthy ({self.consecutive_failures}/"
                f"{self.failure_threshold}) - Error: {error_msg}"
            )
        ...
        while self.is_running:
            try:
                ...
                if (
                    ...
                    logger.error(
                        f"Service has been unhealthy for "
